...
namespace WmsSelfReferral.Controllers
{
    ...
    public class SelfReferralController : SessionControllerBase
    {
        ...
        public SelfReferralController(ILogger<SelfReferralController> logger, IWmsReferralService wmsSelfReferralService, IPostcodesioService postcodesioService,
            TelemetryClient telemetry, IWmsCalculations referralBusiness, INhsLoginService nhsLoginService, IGetAddressioService getAddressioService)
        {
            _logger = logger;
                ...
                switch (response.StatusCode)
                {
                    case 200:
                        ...
                        if (keyanswers.QueriedReferral == false) //only use referral data once
                            SetReferralSessionData(response.Referral);
                    ...
                    case 409:
                        ...
                        _ = DateTime.TryParse(response.Errors.GetValueOrDefault("dateOfReferral"), culture, DateTimeStyles.None, out DateTime DateOfRefferal);
        ...
        public async Task<IActionResult> DateofBirth(DateOfBirthViewModel model)
        {
            ...
            //SetReferralSessionData(selfReferral); don't allow dob to be set - nhs login
            ...
            try
            {
                ...
                if (result.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    referral.ProviderChoices.Add(new Provider()
                    {
                        ...
                        Summary = "If you need more time to choose a service, select this option and we'll send you a text message within 2 working days. This message, from ‘NHS DWMP’, will contain a link back to this website from which you can access the list of available programmes. "
        ...
        private SelfReferral GetReferralSessionData()
        {
            try
            {
                var srSession = HttpContext.Session.Get<SelfReferral>(SESSION_KEY_SELFREFERRAL_TOKEN);
                if (srSession == null) //if its null midway may need to start again
                    return new SelfReferral { ReferringGpPracticeNumber = "V81998" };
                return srSession;
            }
            catch (Exception ex)
            {
        ...
        private void SetReferralSessionData(SelfReferral selfReferral)
        {
            try
            {
                HttpContext.Session.Set<SelfReferral>(SESSION_KEY_SELFREFERRAL_TOKEN, selfReferral);
