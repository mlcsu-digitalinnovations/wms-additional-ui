...
namespace WmsReferral.Tests.PharmacyReferral.Controllers
{
    public class PharmacyReferralControllerTests
    {
        ...
        public void ConsentNHSNUmberGP_ReturnsInvalidView(string referringOdsCode)
        {
            ...
            //setup session/context
            _classUnderTest.ControllerContext.HttpContext = TestSetup.InitializePharmacyControllerContext(pharmacyReferral, new KeyAnswer()).Object;
        ...
        public void GPPracticeConfirmPost_ReturnsValidView()
        {
            ...
            //Assert
            var outputResult = Assert.IsType<RedirectToActionResult>(result);
            Assert.Equal("email-address", outputResult.ActionName);
        ...
        [Fact]
        public void Hypertension_ReturnsView()
        {
            //Arrange                        
            WmsReferral.Business.Models.PharmacyReferral pharmacyReferral = new() { };
            ...
            //Act
            var result = _classUnderTest.Hypertension();

            //Assert
            var outputResult = Assert.IsType<ViewResult>(result);
            Assert.NotNull(((HypertensionViewModel)outputResult.ViewData.Model).HypertensionList);
        }
        [Theory]
        [InlineData("false")]
        [InlineData("true")]
        public void HypertensionPost_ReturnsView(string consent)
        {
            //Arrange                        
            WmsReferral.Business.Models.PharmacyReferral pharmacyReferral = new() { };
            HypertensionViewModel model = new() { Hypertension = consent };
            //setup session/context
            _classUnderTest.ControllerContext.HttpContext = TestSetup.InitializePharmacyControllerContext(pharmacyReferral, new KeyAnswer()).Object;

            //Act
            var result = _classUnderTest.Hypertension(model);

            //Assert
            var outputResult = Assert.IsType<RedirectToActionResult>(result);
            Assert.Equal("Diabetes-Type-One", outputResult.ActionName);
        }
        [Fact]
        public void Diabetes1_ReturnsView()
        {
            //Arrange                        
            WmsReferral.Business.Models.PharmacyReferral pharmacyReferral = new() { };
            ...
            //Act
            var result = _classUnderTest.DiabetesTypeOne();

            //Assert
            var outputResult = Assert.IsType<ViewResult>(result);
            Assert.NotNull(((DiabetesViewModel)outputResult.ViewData.Model).DiabetesList);
        }
        [Theory]
        [InlineData("false")]
        [InlineData("true")]
        public void Diabetes1Post_ReturnsView(string consent)
        {
            //Arrange                        
            WmsReferral.Business.Models.PharmacyReferral pharmacyReferral = new() { };
            DiabetesViewModel model = new() { DiabetesTypeOne = consent };
            //setup session/context
            _classUnderTest.ControllerContext.HttpContext = TestSetup.InitializePharmacyControllerContext(pharmacyReferral, new KeyAnswer()).Object;

            //Act
            var result = _classUnderTest.DiabetesTypeOne(model);

            //Assert
            var outputResult = Assert.IsType<RedirectToActionResult>(result);
            Assert.Equal("diabetes-type-two", outputResult.ActionName);
        }
        [Fact]
        public void Diabetes2_ReturnsView()
        {
            //Arrange                        
            WmsReferral.Business.Models.PharmacyReferral pharmacyReferral = new() { Email="" };

            //setup session/context
            _classUnderTest.ControllerContext.HttpContext = TestSetup.InitializePharmacyControllerContext(pharmacyReferral, new KeyAnswer()).Object;

            //Act
            var result = _classUnderTest.DiabetesTypeTwo();

            //Assert
            var outputResult = Assert.IsType<ViewResult>(result);
            Assert.NotNull(((DiabetesTypeTwoViewModel)outputResult.ViewData.Model).DiabetesList);
        }
        [Theory]
        [InlineData("false")]
        [InlineData("true")]
        public void Diabetes2Post_ReturnsView(string consent)
        {
            //Arrange                        
            WmsReferral.Business.Models.PharmacyReferral pharmacyReferral = new() { Email="somethin@nhs.net" };
            DiabetesTypeTwoViewModel model = new() { DiabetesTypeTwo = consent };
            //setup session/context
            _classUnderTest.ControllerContext.HttpContext = TestSetup.InitializePharmacyControllerContext(pharmacyReferral, new KeyAnswer()).Object;

            //Act
            var result = _classUnderTest.DiabetesTypeTwo(model);
        ...
        public void GivenNamePost_ReturnsSuccessAction(string answer)
        {
            ...
            //Assert
            var outputResult = Assert.IsType<RedirectToActionResult>(result);
            Assert.Equal("Address", outputResult.ActionName);
        }
        [Fact]
        ...
        [Fact]
        public void VulnerablePatientGet_ReturnsValidView()
        {
            //Arrange                    
            WmsReferral.Business.Models.PharmacyReferral referral = new() { Email = "something@nhsorg.nhs.uk" };
            ...
            //Act
            var result = _classUnderTest.VulnerablePatient();

            //Assert
            var outputResult = Assert.IsType<ViewResult>(result);
            Assert.NotNull(((VulnerablePatientViewModel)outputResult.ViewData.Model).VulnerablePatientList);
        }
        [Fact]
        public void CheckAnswersGet_ReturnsView()
        {
            ...
            //setup session/context
            _classUnderTest.ControllerContext.HttpContext = TestSetup.InitializePharmacyControllerContext(referral, keyAnswer).Object;
