...
namespace WmsReferral.Business.Services
{
    public interface IWmsReferralService
    {
        ...
        Task<HttpResponseMessage> GeneratePharmacyTokenAsync(string email, int expireMinutes);
        Task<HttpResponseMessage> ValidatePharmacyTokenAsync(string email, string token);
