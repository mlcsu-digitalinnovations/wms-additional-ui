...
namespace WmsReferral.Business.Shared
{
    public class WmsCalculations : IWmsCalculations
    {
        ...
        public decimal? ConvertFeetInches(int? feet, decimal? inches)
        {
            try
            {
                return Math.Round((decimal)((decimal)(feet * 30.48) + (inches * (decimal)2.54)), 1);
            }
            catch (Exception ex)
            {
        ...
        public HeightImperialViewModel ConvertCm(decimal? cm)
        {
            try
            {
                if (cm != null)
                {
                    ...
                    inches = Math.Round((decimal)inches, 1, MidpointRounding.AwayFromZero);
                    if (inches == 12)
                    { //rounded up
                        inches = 0;
                        feet += 1;
        ...
        public WeightImperialViewModel ConvertKg(decimal? kg)
        {
            try
            {
                if (kg != null)
                {
                    ...
                    pounds = Math.Round((decimal)pounds, 1, MidpointRounding.ToZero);
