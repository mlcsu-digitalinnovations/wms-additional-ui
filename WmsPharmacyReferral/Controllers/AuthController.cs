...
{
    [Host("localhost", "pharmacy.wms-pre.mlcsu.org", "pharmacy.wmp.nhs.uk")]
    public class AuthController : SessionControllerBase
    {
        ...
        private async Task<AuthViewModel> GenerateToken()
        {
            ...
            try
            {
                ...
                var authToken = await _WmsSelfReferralService.GeneratePharmacyTokenAsync(authmodel.EmailAddress, tokenlifetime);
        ...
        public async Task<IActionResult> ValidateCode(AuthViewModel model)
        {
            ...
            {
                //request auth token from API
                var authToken = await _WmsSelfReferralService.ValidatePharmacyTokenAsync(model.EmailAddress, model.Token);
